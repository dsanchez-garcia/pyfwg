...
class _MorphingWorkflowBase:
    ...
    def __init__(self, tool_scenarios: List[str], valid_models: set, model_arg_name: str):
    ...
    def preview_rename_plan(self,
        ...
        This method performs a non-destructive "dry run". It iterates through
        all mapped files and simulates the final filenames that will be created
        for every possible scenario and year.

        It performs a critical validation to ensure that the output pattern
        will result in unique filenames, preventing data loss from overwritten
        files.

        Args:
            final_output_dir (str): The path to the directory where the final,
                renamed files will be moved.
            output_filename_pattern (str): A template string for the final
                filenames. It MUST contain the placeholders `{ssp}` and `{year}`
        ...
        # Validate that the pattern contains required placeholders to prevent overwrites.
        ...
        # Pre-compute which placeholders must come from the mapping step.
        ...
        for epw_path, mapped_data in all_mapped_files.items():
            ...
            # Validate that this file has all the categories needed for the output pattern.

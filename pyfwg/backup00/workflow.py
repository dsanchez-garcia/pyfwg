...
class _MorphingWorkflowBase:
    ...
    def preview_rename_plan(self,
        ...
        specific FutureWeatherGenerator tool.

        It performs two critical validations:
        1. It checks that the `output_filename_pattern` contains the required
           placeholders (`{year}` and a tool-specific scenario placeholder like
           `{ssp}` or `{rcp}`) to prevent data loss from overwritten files.
        2. For each file, it verifies that the categories extracted in Step 1
           can satisfy all other placeholders in the output pattern.

        If a file is missing a required category, it will print a clear error
        message for that file and exclude it from the final execution plan.
        ...
        Args:
            ...
            output_filename_pattern (str): A template string for the final
                filenames. It MUST contain the placeholder `{year}` and a scenario
                placeholder (e.g., `{ssp}` for the global tool, `{rcp}` for the
                Europe tool) to ensure unique filenames.
            ...
            scenario_mapping (Optional[Dict[str, str]], optional): A dictionary
                ...
                descriptive format (e.g., 'SSP1-2.6') for the scenario
        ...
        # --- Validation 1: Ensure pattern contains required dynamic placeholders ---
        ...
        if required_placeholder not in output_filename_pattern or '{year}' not in output_filename_pattern:
            raise ValueError(
                f"The 'output_filename_pattern' must contain both '{required_placeholder}' and '{{year}}' placeholders "
        ...
        # Reset the rename plan to ensure the method is idempotent (rerunnable).
        self.rename_plan = {}
        # Combine complete and incomplete files to provide a full preview for the user.
        all_mapped_files = {**self.epw_categories, **self.incomplete_epw_categories}

        # --- Pre-computation for Validation 2 ---
        ...
        # Define the placeholders that are generated automatically by this script,
        # including the dynamic scenario placeholder.
        auto_placeholders = {'scenario', self.scenario_placeholder_name, 'year'}
        # Determine which placeholders MUST be provided by the user's mapping step.
        ...
        for epw_path, mapped_data in all_mapped_files.items():
            ...
            # --- Validation 2: Check if this file has all needed categories ---
            ...
            # Iterate through all possible outputs the specific tool will create.
            for year in ALL_POSSIBLE_YEARS:
                for scenario in self.tool_scenarios:  # Use the instance attribute for scenarios
                    # Create a complete data dictionary for formatting the filename.
                    filename_data = {
                        **mapped_data,
                        'scenario': scenario,  # The raw scenario name (e.g., 'ssp126')
                        self.scenario_placeholder_name: self.inputs['scenario_mapping'].get(scenario, scenario),  # The mapped name (e.g., 'SSP1-2.6')
                        'year': year
                    }
    ...
    def _set_morphing_config_base(self, *,
                                  fwg_jar_path: str,
                                  ...
                                  **kwargs):
        """(Private) Base method for setting and validating configuration.

        This method handles the core logic of merging parameters, validating them,
        and preparing the final state for the `execute_morphing` step.
        """
        # Start with the base dictionary, or an empty one if not provided.
        final_fwg_params = fwg_params.copy() if fwg_params else {}

        # Create a dictionary of all other keyword arguments to override the base.
        overrides = kwargs
        # Add the generic 'models' argument, which will be either GCMs or RCM pairs.
        overrides[self.model_arg_name] = fwg_models
        # Apply the overrides.
        ...
        # Print the final configuration for user review.
        ...
        print(f"\n  --- Final Formatted FWG Parameters ({self.model_arg_name}) ---")
...
class MorphingWorkflowGlobal(_MorphingWorkflowBase):
    ...
    def set_morphing_config(self, *,
        ...
        """STEP 3: Sets, validates, and reviews the configuration for the GLOBAL tool.

        This method gathers all parameters for the morphing execution, validates
        them against the known constraints for the global tool, and stores them
        in the instance's state. It provides a final summary for user review.
        ...
        Args:
            ...
            fwg_gcms (Optional[List[str]], optional): A specific list of GCMs
                to use. If None, the full default list is used. Overrides
                `fwg_params`.
            fwg_create_ensemble (bool, optional): If True, creates an ensemble.
                Overrides `fwg_params`.
            fwg_winter_sd_shift (float, optional): Winter standard deviation
                shift. Overrides `fwg_params`.
            fwg_summer_sd_shift (float, optional): Summer standard deviation
                shift. Overrides `fwg_params`.
            fwg_month_transition_hours (int, optional): Hours for month
                transition. Overrides `fwg_params`.
            fwg_use_multithreading (bool, optional): Use multithreading.
                Overrides `fwg_params`.
            fwg_interpolation_method_id (int, optional): Interpolation method
                ID. Overrides `fwg_params`.
            fwg_limit_variables (bool, optional): Limit variables to physical
                bounds. Overrides `fwg_params`.
            fwg_solar_hour_adjustment (int, optional): Solar hour adjustment
                option. Overrides `fwg_params`.
            fwg_diffuse_irradiation_model (int, optional): Diffuse irradiation
                model option. Overrides `fwg_params`.
            fwg_add_uhi (bool, optional): Add UHI effect. Overrides `fwg_params`.
            fwg_epw_original_lcz (int, optional): Original EPW LCZ.
                Overrides `fwg_params`.
            fwg_target_uhi_lcz (int, optional): Target UHI LCZ.
                Overrides `fwg_params`.
        """
        # Collect all other keyword arguments to pass to the base method.
        other_fwg_params = {
        ...
        # Call the base configuration method, passing the specific model
        # argument ('fwg_gcms') as the generic 'fwg_models'.
        super()._set_morphing_config_base(
            fwg_jar_path=fwg_jar_path,
            run_incomplete_files=run_incomplete_files,
            delete_temp_files=delete_temp_files,
            temp_base_dir=temp_base_dir,
...
class MorphingWorkflowEurope(_MorphingWorkflowBase):
    ...
    def set_morphing_config(self, *,
                            ...
                            fwg_epw_original_lcz: int = 14,
                            fwg_target_uhi_lcz: int = 1):
        """STEP 3: Sets, validates, and reviews the configuration for the EUROPE tool.

        This method gathers all parameters for the morphing execution, validates
        them against the known constraints for the Europe-specific tool, and
        stores them in the instance's state. It provides a final summary for
        user review.
        ...
        # Call the base configuration method, passing the specific model
        # argument ('fwg_rcm_pairs') as the generic 'fwg_models'.
        super()._set_morphing_config_base(
            fwg_jar_path=fwg_jar_path,
            run_incomplete_files=run_incomplete_files,
            delete_temp_files=delete_temp_files,

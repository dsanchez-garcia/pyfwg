...
class MorphingIterator:
    ...
    **Step 6: Execute the Batch Run**
    ...
    def generate_morphing_workflows(self,
        ...
        Raises:
            ValueError: If `raise_on_overwrite` is True and a potential file
        ...
        if missing_placeholders:
            ...
            # Check the flag to decide whether to raise an error or issue a warning.
        ...
        for index, row in plan_df.iterrows():
            ...
            for year in ALL_POSSIBLE_YEARS:
                for scenario in self.workflow_class.tool_scenarios:
                    ...
                    # This variable needs to be defined before the try block.
                    final_filename = None
                    try:
                        # Generate the final filename.
                        final_filename = output_pattern.format(**filename_data)
                        # If the filename has been seen before, it's a collision.
                        if final_filename in generated_filenames:
                            message = (
                                f"Definitive file overwrite detected! The filename '{final_filename}.epw' is generated by more than one run. "
                                "Please review your `output_filename_pattern` and `keyword_mapping`."
                            )
                            if raise_on_overwrite:
                                raise ValueError(message)
                            else:
                                ...
                                break  # Stop checking this run as a collision was found
                ...
                # If a collision was found and we are not raising an error, stop checking this run.
                if final_filename in generated_filenames and not raise_on_overwrite:
                    break

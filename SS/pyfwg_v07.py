...
class MorphingWorkflow:
            ...
            if file_categories:
                ...
                if keyword_mapping:
                    ...
                    if len(found_categories) < len(all_defined_categories):
                        ...
                        logging.warning(f"File '{os.path.basename(epw_path)}' is missing categories: {list(missing)}. It will be processed only if 'run_incomplete_files' is True.")
            ...
            for year in ALL_POSSIBLE_YEARS:
                for scenario in ALL_POSSIBLE_SCENARIOS:
                    ...
                    try:
                        ...
                        print(f"    -> ERROR: Placeholder {e} in output pattern was not found in this file's mapped categories. Renaming will fail.")
                        break
    ...
    def execute_morphing(self, *,
        ...
        """
        STEP 3: Executes the morphing process with explicit parameters.

        :param fwg_jar_path: Path to the FutureWeatherGenerator .jar file.
        :param run_incomplete_files: If True, also processes files that were only partially categorized. Defaults to False.
        :param delete_temp_files: If True, deletes temporary folders after processing.
        :param temp_base_dir: The base directory for temporary morphing results.
        :param fwg_gcms: List of GCMs to use. If None, the tool's default is used.
        :param fwg_create_ensemble: If True, creates an ensemble from the selected GCMs.
        :param fwg_winter_sd_shift: Standard deviation shift for winter peak month.
        :param fwg_summer_sd_shift: Standard deviation shift for summer peak month.
        :param fwg_month_transition_hours: Hours to smooth the transition between months.
        :param fwg_use_multithreading: Use multithread computation.
        :param fwg_interpolation_method_id: Grid method ID (0, 1, or 2).
        :param fwg_limit_variables: Bound variables to physical limits.
        :param fwg_solar_hour_adjustment: Solar hour adjustment option (0, 1, or 2).
        :param fwg_diffuse_irradiation_model: Diffuse solar irradiation model option (0, 1, or 2).
        :param fwg_uhi_options: String for Urban Heat Island settings.
        """
        ...
        # Collect all fwg_ arguments into a clean dictionary
        fwg_params = {
        ...
        self.inputs.update({
            'fwg_jar_path': fwg_jar_path, 'fwg_params': fwg_params,
            'delete_temp_files': delete_temp_files, 'temp_base_dir': temp_base_dir
        })

        # Determine which files to process based on the flag
        files_to_process = list(self.epw_categories.keys())
        if run_incomplete_files:
            logging.info("Processing both complete and incomplete files.")
    ...
    def _execute_single_morph(self, epw_path: str, temp_output_dir: str) -> bool:
        ...
        command = [
            ...
            ",".join(params.get('gcms') or DEFAULT_GCMS),
            '1' if params.get('create_ensemble') else '0',
            f"{params.get('winter_sd_shift')}:{params.get('summer_sd_shift')}",
            str(params.get('month_transition_hours')),
            os.path.abspath(temp_output_dir) + os.sep,
            str(params.get('use_multithreading')).lower(),
            str(params.get('interpolation_method_id')),
            str(params.get('limit_variables')).lower(),
            str(params.get('solar_hour_adjustment')),
            str(params.get('diffuse_irradiation_model')),
            params.get('uhi_options')
        ]
...
#         epw_files=[
#             'sevilla_uhi_tipo-1.epw',
#             'este_epw_es_de_sevilla.epw',  # Incomplete file
#         ],
#         keyword_mapping={
...
#         final_output_dir='./final_results_final',
#         output_filename_pattern='{city}_{uhi}_{ssp_full_name}_{year}',
#         scenario_mapping={'ssp126': 'SSP1-2.6'}
#     )
#
#     # To execute, call the method with explicit, prefixed arguments
#     # workflow.execute_morphing(
#     #     fwg_jar_path='path/to/your.jar',
#     #     run_incomplete_files=True, # Set to True to process the incomplete file

...
class MorphingWorkflow:
    ...
    def map_categories(self,
        ...
        if input_filename_pattern and keyword_mapping: raise ValueError("Please provide either 'input_filename_pattern' or 'keyword_mapping', not both.")
        if not input_filename_pattern and not keyword_mapping: raise ValueError("You must provide one mapping method: 'input_filename_pattern' or 'keyword_mapping'.")
    ...
    def preview_rename_plan(self,
        ...
        """
        STEP 2: Generates a plan for all mapped files with robust validation.
        """
        ...
        # Find all placeholders like {city}, {uhi}, etc. in the pattern
        required_placeholders = set(re.findall(r'{(.*?)}', output_filename_pattern))
        # These are added automatically, so they are not required from the mapping step
